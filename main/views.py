...
from .models import Register, Contact, Tap_model, FullStack, DigitalMarket
...
def Digital_Market(request):
    if request.method == 'POST':
        ...
        femails = request.POST['femails']
        fphones = request.POST['fphones']
        passout = request.POST['passout']
        city = request.POST['city']
        classes = request.POST['classes']
        
        comment = request.POST['comment']
        digitalmarket = DigitalMarket(
            fname=fname, femails=femails, fphones=fphones, passout=passout,
            city=city, classes=classes,  comment=comment, )
        digitalmarket.save()
...
def full_stack(request):
    if request.method == 'POST':
        ...
        city = request.POST['city']
        classes = request.POST['classes']
        upcomingBatch = request.POST['upcomingBatch']
        comment = request.POST['comment']
        fullstack = FullStack(
            fname=fname, femails=femails, fphones=fphones, passout=passout,
            city=city, classes=classes, upcomingBatch=upcomingBatch, comment=comment, )
        fullstack.save()
